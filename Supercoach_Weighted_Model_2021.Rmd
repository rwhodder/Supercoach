...
all_factors <- merge(all_factors, consistency)
...
complete_data <- left_join(price_data, final_updated, by = "helper2")
...
fwd <- complete_data %>% dplyr::filter(Position == "FWD")
def <- complete_data %>% dplyr::filter(Position == "DEF")
mid <- complete_data %>% dplyr::filter(Position == "MID")
ruc <- complete_data %>% dplyr::filter(Position == "RUC")
```
...
proj_final  <- proj_price %>% mutate(proj_salary     = round(proj_salary * 0.90, 0),
  ...
  select(c(Name, Team, Position, proj_score, proj_salary, Price, value, `2020_Games`, `2020_Average`,
           ...
           weighted_durability, weighted_form, weighted_consistency)) %>%
...
write.csv(proj_final, "projections_2021.csv")
```
